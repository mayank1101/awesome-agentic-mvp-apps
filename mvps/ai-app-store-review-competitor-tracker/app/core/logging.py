...
call sites that leak keys are the ones nobody thought about -- a provider
error whose message quotes the request, a debug line dumping settings. A
filter covers those too.
"""
...
#: Groq keys are `gsk_`, OpenAI/OpenRouter are `sk-`; the generic fallback
#: catches `api_key=...` style leaks from third-party error strings.
...
def redact_secrets(text: str) -> str:
    ...
    Returns:
        ...
        redactor that can fail is a redactor that will fail at the worst
        moment.
    """

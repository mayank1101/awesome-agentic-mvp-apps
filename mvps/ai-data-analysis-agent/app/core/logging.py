...
message quotes the request body. Unlike the resume app in this repo, this app's
input is arbitrary tabular data rather than a named person's contact details, so
the filter here only covers credential shapes, not PII patterns.
"""
...
#: Groq keys are `gsk_`; the generic `api_key=...` pattern covers other
#: providers' shapes when a third-party error string quotes the request.

...
Tavily, over plain `httpx`. One endpoint, one request shape, so the official
SDK would be a dependency that saves nothing.

Everything network-shaped that can go wrong is classified here, once, into
this app's own error vocabulary. Tavily returns HTTP 429 both for "you are
going too fast" and for "your monthly credits are gone", and those need
opposite handling -- the first clears in seconds, the second does not clear at
all this month.
"""
...
def post_json(path: str, payload: dict[str, Any]) -> dict[str, Any]:
    ...
    Raises:
        ...
        SearchError: Anything else -- network failure, timeout, 5xx, or a
            reply that was not a JSON object.
    """
...
def _safe_body(response: httpx.Response) -> str:
    ...
    Capped: an error body is occasionally an entire HTML page, and the only
    part that carries a marker is the beginning.
    """
    try:

...
Streamlit runs the script on its own thread and this app makes exactly one
model call per run, so there is nothing an event loop would buy.

Provider errors are classified by status code and message, not by SDK
exception class, because the classes move between SDK versions and `429` does
not. JSON is asked for twice -- through `response_format` and again in words --
and a reply that still fails to parse gets exactly one repair retry that shows
...
def _client() -> Any:
    ...
    Imported lazily so that importing `app` -- which the offline test suite
    does constantly -- does not require the SDK to be installed or a key set.
...
def _classify(exc: Exception) -> ModelError:
    ...
    if status == 429:
        ...
        if figures and int(figures.group(2)) > int(figures.group(1)):
            return ModelRequestTooLarge(
                ...
                f"limit is {figures.group(1)}. Try a shorter trip."
            )
    ...
    if status == 413 or "too large" in lowered or "context_length" in lowered:
        return ModelRequestTooLarge(
            "The request was too large for this model's limits. Try a shorter trip."
        )
...
def complete_model(
    *,
    ...
    Returns:
        The validated instance, and whether the repair retry was needed.
    ...
    try:
        return schema.model_validate(parse_json(raw)), False
    ...
    try:
        return schema.model_validate(parse_json(raw)), True

...
around it, so they are created per request (their instructions vary with the
brief) while the client -- and its connection pool -- is shared.
    ...
    *,
    ...
    Args:
        response_format: Structured-output request, as produced by
            :func:`outline_response_format`. Omitted entirely when ``None``.
...
def outline_response_format(schema: type[BaseModel]) -> Any | None:
    ...
    Args:
        schema: The Pydantic model the outline should conform to.

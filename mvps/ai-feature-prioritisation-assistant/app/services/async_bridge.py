...
That is not incidental: the HTTP client underneath the cached chat client keeps
a connection pool bound to the loop that created it. A per-call
``asyncio.run()`` would work for the first estimation and fail on the second,
once the loop that owns the pool has been closed.
...
_LOOP_THREAD_NAME = "prioritisation-agent-loop"
...
def run_sync(coro: Coroutine[object, object, T]) -> T:
    ...
    There is no streaming twin of this function, unlike the other apps in this
    repo. The estimator's reply is a structured object that has to validate
    before any of it means anything -- half a JSON object is not half a ranking

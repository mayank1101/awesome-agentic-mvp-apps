...
message quotes the request body, a debug line dumping a parsed profile, a search
URL echoed back with the key in the query string.

This app handles a **real person's resume**. Names, emails, and phone numbers
pass through every layer. Nothing in `app/` logs resume text, and the filter
below catches the email and phone shapes anyway, because "nothing logs it" is a
claim about code that will be edited later.
"""
...
#: Credential shapes this app handles. Groq keys are ``gsk_`` and Tavily keys are
#: ``tvly-``, both matchable by shape. Mistral keys are a bare 32-char token and
#: are not, which is what the generic ``api_key=...`` pattern is for -- it covers
#: the case where a third-party error string quotes the request that carried it.
...
def configure_logging() -> None:
    ...
    per request, and this app makes a lot of them -- several searches, an extract
    batch, one model call per scored job -- which drowns out its own logs at
    DEBUG.
    """
    global _CONFIGURED
    if _CONFIGURED:

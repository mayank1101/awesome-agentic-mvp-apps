"""Tests for resume intake and posting-text preparation.
...
def test_posting_text_is_capped_on_a_line_boundary(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("MAX_POSTING_CHARS", "80")
    get_settings.cache_clear()

    text, truncated = pdf_extract.prepare_posting_text(
        "\n".join(f"line {i}" * 3 for i in range(20))
    )

    assert truncated
    assert len(text) <= 80
    # Never mid-line: half a bullet is something a model completes from
    # imagination.
    assert not text.endswith("lin")


def test_a_short_posting_is_untouched() -> None:
    text, truncated = pdf_extract.prepare_posting_text(

...
Ported from `ai-prd-generator`. Groq is the default here rather than one option
among several, but the registry is kept whole: the repo's apps share it, and an
...
#: map have no schema-less JSON mode; for those, the shape is carried by the
#: instructions alone, which is why the synthesiser validates rather than trusts.
...
def get_chat_client() -> SupportsChatGetResponse:
    ...
    Raises:
        ImportError: The selected provider's optional package is not installed,
            re-raised with the package to install.
    """
    settings = get_settings()
    logger.info(
...
def json_response_format() -> Any | None:
    ...
    Returns:
        The provider's ``response_format`` value, or ``None`` for providers with
        no schema-less JSON mode -- for those the shape lives in the instructions
        and is enforced by validating the reply.
    """
...
def build_options(*, max_tokens: int | None = None, as_json: bool = True) -> ChatOptions:
    """Assemble the per-call options for the synthesis run.

    Args:
        max_tokens: Output cap for this call. Defaults to the configured value.
        as_json: Whether to request JSON. The repair retry keeps this on; only a
            plain-text probe would turn it off.

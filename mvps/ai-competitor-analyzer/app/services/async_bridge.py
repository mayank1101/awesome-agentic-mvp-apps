...
``asyncio.run()`` would work for the first section and fail on the second, once
...
_LOOP_THREAD_NAME = "analyzer-agent-loop"
...
def iter_sync(agen_factory: Callable[[], AsyncIterator[T]]) -> Iterator[T]:
    ...
    Section bodies are a few kilobytes, so the memory ceiling is not a concern.
    ...
    Raises:
        Exception: Anything the async iterator raised, re-raised on the
            caller's thread once the preceding items have been yielded.
    """
    queue: Queue = Queue()
    ...
    asyncio.run_coroutine_threadsafe(pump(), _get_loop())
